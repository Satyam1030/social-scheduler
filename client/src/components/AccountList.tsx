...
const AccountList=({accounts,onDisconnect}:AccountListProps)=>{
    ...
    if(accounts.length===0){
        return(
            <div className="bg-white rounded-2xl border-2 border-dashed border-slate-200 flex flex-col items-center justify-centerpy-20 px-6">
    ...
    return(
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {accounts.map((account,index)=>{
                ...
                return (
                    <div key={index} className="group bg-white border border-slate-200 rounded-2xl p-5 flex items-center gap-4 hover:bg-slate-300 transition-all">
                        ...
                        <div className="flex items-center gap-1.5 shrink-0">
                            ...
                            <span className="size-xs text-amber-600">Disconnected</span>
                            </>}
                        </div>
