...
const AIComposer=()=>{
    ...
    const [selectedPlatorms,setSelectedPlatforms]=useState<string[]>([]);
    ...
    const handleSchedule=async()=>{
        ...
        if(selectedPlatorms.length===0){
            toast.error("Select atleast one plaform");
            return;
        }
        ...
        <div className="max-w-4xl mx-auto space-y-12 pb-20 animate-in fade-in duration-700">
            ...
            <div className="space-y-6 text-center mt-20">
                ...
                <div className="relative group mt-12">
                    ...
                    value={prompt} onChange={(e)=>setPrompt(e.target.value)} className="w-full px-6 py-6 bg-white border border-slate-300 
                    rounded-xl text-slate-900 placeholder-slate-400 outline-none focus:border-slate-400 transition resize-none h-40"/>
                    ...
                    <div className="absolute bottom-4 right-2.5 flex items-center gap-3 text-sm">
                        ...
                        <button onClick={handleGenerate} disabled={loading} className="bg-slate-900 hover:bg-slate-800 text-white flex items-center gap-2 px-4 py-2
                        rounded-lg">
                            {loading?(
                                <>
            ...
            <div className="space-y-6 pt-12 border-t border-slate-100">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                    {generations.map((gen)=>(
                        <div key={gen._id} className="group bg-white rounded-2xl border border-slate-100 p-5 hover:border-red-200 
                    ...
                    <div className="col-span-full py-20 text-center space-y-2">
                        ...
                        <p className="text-slate-400 text-sm">No content generated yet.Try generationg some content using AI.</p>
                    </div>)}
                    ...
                    <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl border border-slate-100 overflow-hidden flex flex-col max-h-[90vh]">
                        ...
                        <div className="flex-1 overflow-y-auto p-8 space-y-4">
                            ...
                            <div className="bg-slate-50 rounded-2xl p-6 border border-slate-100 space-y-4">
                                ...
                                {activeScheduler.mediaUrl && <img src={activeScheduler.mediaUrl} alt="preview" className="w-full aspect-video
                                object-cover rounded-xl border border-slate-200 shadow-sm"/>}
                            </div>
                        </div>
                            ...
                            <div className="space-y-6">
                                <div>
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                        {PLATFORMS.map((p)=>{
                                            const active=selectedPlatorms.includes(p.id)
                                            return (<button key={p.id} onClick={()=>setSelectedPlatforms((prev)=>(prev.includes(p.id) 
                                            ? prev.filter((x)=>x!=p.id) : [...prev,p.id]))} className={`p-2.5 rounded-md border text-xs
                                            ${active ?"bg-red-500/80 text-white":"bg-white border-slate-200 text-slate-400 hover:border-slate-300"}`}>
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    <div className="relative">
                                        ...
                                        <input type="date" className="w-full pl-11 pr-4 py-3 bg-slate-50 border border-slate-100 rounded-md
                                        text-slate-900 text-sm focus:outline-none transition-all" value={scheduledDate} onChange={(e)=>setScheduledDate(e.target.value)}/>
                                    </div>

                                    <div className="relative">
                                        <ClockIcon className="size-4 absolute left-4 top-1/2 -translate-y-1/2 text-slate-400"/>
                                        <input type="time" className="w-full pl-11 pr-4 py-3 bg-slate-50 border border-slate-100 rounded-md
                                        text-slate-900 text-sm focus:outline-none transition-all" value={scheduledTime} onChange={(e)=>setScheduledTime(e.target.value)}/>
                                    </div>
                                </div>
                            </div>

                            <button onClick={handleSchedule} className="w-full flex items-center justify-center gap-2 py-3 rounded-md bg-slate-200 text-slate-700 hover:bg-red-500
